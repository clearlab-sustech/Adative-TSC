...
double q_init[] = {0.0, -3.0, 0.357, 1, 0, 0, 0,
                   0.577, 1.21, -2.78,
                   -0.577, 1.21, -2.78,
                   0.577, 1.21, -2.78,
                   -0.577, 1.21, -2.78};

bool simIsDone = false;

std::queue<JointsCmd_lcm> ctrl_quene;
JointsCmd_lcm current_ctrl_law;
// lcm message
...
std::array<Plane, 13> planes;
PerceptionTerrain_lcm lcm_planes;
PerceptionLocation_lcm lcm_location;
...
void loadmodel(void)
{
    ...
    for (int i = 0; i < 12; i++)
    {
        ...
        one_plane.Bs[0] = Eigen::Vector3d(1, 0, 0.65 + (i + 1) * stair_d - 0);
        one_plane.Bs[1] = Eigen::Vector3d(0, -1, stair_l / 2);
        one_plane.Bs[2] = Eigen::Vector3d(-1, 0, -(0.65 + i * stair_d));
        one_plane.Bs[3] = Eigen::Vector3d(0, 1, stair_l / 2);
        planes[i] = one_plane;
    }
    planes[12].A << 0, 0, stair_h * (1 + 5);
    planes[12].bound_num = 4;
    planes[12].Bs[0] = Eigen::Vector3d(1, 0, 0.65 + 0.3 + (5 + 1) * stair_d - 0);
    planes[12].Bs[1] = Eigen::Vector3d(0, -1, stair_l / 2 + 0.2);
    planes[12].Bs[2] = Eigen::Vector3d(-1, 0, -(0.65 + 5 * stair_d));
    planes[12].Bs[3] = Eigen::Vector3d(0, 1, stair_l / 2);

    // planes[0].A << -0.0315797, 0.00977286, 0.0218844;
    // planes[0].bound_num = 3;
    // planes[0].Bs[0] << -32.7974, -59.8005, -0.933674;
    // planes[0].Bs[1] << -55.7741, 0.0828247, -37.4738;
    // planes[0].Bs[2] << 88.5715, 59.7177, 41.7455;
    // planes[0].Vertexs[0] << 0.731162, -0.38539;
    // planes[0].Vertexs[1] << 0.671361, -0.352593;
    // planes[0].Vertexs[2] << 0.671444, -0.296819;

    // planes[1].A << -0.0315797, 0.00977286, 0.0218844;
    // planes[1].bound_num = 4;
    // planes[1].Bs[0] << -113.094, -206.208, -3.21949;
    // planes[1].Bs[1] << -436.753, 8.12646, -205.103;
    // planes[1].Bs[2] << -104.714, 199.054, -10.295;
    // planes[1].Bs[3] << 654.561, -0.972107, 439.79;
    // planes[1].Vertexs[0] << 0.671361, -0.352593;
    // planes[1].Vertexs[1] << 0.465153, -0.239499;
    // planes[1].Vertexs[2] << 0.473279, 0.197254;
    // planes[1].Vertexs[3] << 0.672333, 0.301968;

    // planes[2].A << -0.0290749, 0.00774652, 0.0168024;
    // planes[2].bound_num = 4;
    // planes[2].Bs[0] << 153.57, 40.6907, 198.375;
    // planes[2].Bs[1] << -397.302, -711.258, -13.599;
    // planes[2].Bs[2] << -6.02161, 256.307, -114.297;
    // planes[2].Bs[3] << 249.753, 414.26, 89.0902;
    // planes[2].Vertexs[0] << 1.46952, -0.670898;
    // planes[2].Vertexs[1] << 1.51021, -0.824468;
    // planes[2].Vertexs[2] << 0.798951, -0.427166;
    // planes[2].Vertexs[3] << 1.05526, -0.421145;

    // planes[3].A << -0.0300101, 0.00936738, 0.117358;
    // planes[3].bound_num = 4;
    // planes[3].Bs[0] << -11.4107, -198.97, 48.0387;
    // planes[3].Bs[1] << -514.234, 3.67426, -348.957;
    // planes[3].Bs[2] << -73.9323, 204.667, -2.40239;
    // planes[3].Bs[3] << 599.577, -9.37103, 527.701;
    // planes[3].Vertexs[0] << 0.875563, -0.291649;
    // planes[3].Vertexs[1] << 0.676593, -0.280238;
    // planes[3].Vertexs[2] << 0.680267, 0.233996;
    // planes[3].Vertexs[3] << 0.884935, 0.307928;

    // planes[4].A << -0.00871758, 0.00706644, 0.195664;
    // planes[4].bound_num = 4;
    // planes[4].Bs[0] << 5.26981, -123.071, 40.2205;
    // planes[4].Bs[1] << -547.658, 7.68463, -481.541;
    // planes[4].Bs[2] << -41.8516, 147.42, 1.13187;
    // planes[4].Bs[3] << 584.24, -32.0336, 592.337;
    // planes[4].Vertexs[0] << 0.998285, -0.284061;
    // planes[4].Vertexs[1] << 0.875214, -0.289331;
    // planes[4].Vertexs[2] << 0.882899, 0.258327;
    // planes[4].Vertexs[3] << 1.03032, 0.300179;

    // planes[5].A.setZero();
    // planes[5].bound_num = 4;
    // planes[5].Bs[0] << 0, 1, 1;
    // planes[5].Bs[1] << 1, 0, 0.65;
    // planes[5].Bs[2] << 0, -1, 1;
    // planes[5].Bs[3] << -1, 0, 2;
    // planes[5].Vertexs[0] << 0.65, 1;
    // planes[5].Vertexs[1] << 0.65, -1;
    // planes[5].Vertexs[2] << -2, -1;
    // planes[5].Vertexs[3] << -2, 1;
    ...
    // planes[11].A << 0, 0, 0.2;
    // planes[11].bound_num = 4;
    // planes[11].Bs[0] = Eigen::Vector3d(0, 1, stair_l/2);
    // planes[11].Bs[1] = Eigen::Vector3d(-1, 0, -(1.15-0.5));
    // planes[11].Bs[2] = Eigen::Vector3d(0, -1, stair_l/2);
    // planes[11].Bs[3] = Eigen::Vector3d(1, 0, 1.25-0.5);

    // planes[10].A << 0, 0, 0.2;
    // planes[10].bound_num = 4;
    // planes[10].Bs[0] = Eigen::Vector3d(0, 1, stair_l/2);
    // planes[10].Bs[1] = Eigen::Vector3d(-1, 0, -(1.35-0.5));
    // planes[10].Bs[2] = Eigen::Vector3d(0, -1, stair_l/2);
    // planes[10].Bs[3] = Eigen::Vector3d(1, 0, 1.50-0.5);

    // planes[9].A << 0, 0, 0.2;
    // planes[9].bound_num = 4;
    // planes[9].Bs[0] = Eigen::Vector3d(0, 1, stair_l/2);
    // planes[9].Bs[1] = Eigen::Vector3d(-1, 0, -(1.65-0.5));
    // planes[9].Bs[2] = Eigen::Vector3d(0, -1, stair_l/2);
    // planes[9].Bs[3] = Eigen::Vector3d(1, 0, 2.5-0.5);

    Eigen::Matrix2d Bounds;
    Vec2 uB;
    for (int n = 0; n < planes.size(); n++)
    {
        if (planes[n].bound_num > 0)
        {
            for (int i = 0; i < planes[n].bound_num; i++)
            {
                Bounds.row(0) = planes[n].Bs[i].head(2);
                Bounds.row(1) = planes[n].Bs[(i + 1) % planes[n].bound_num].head(2);
                uB << planes[n].Bs[i][2], planes[n].Bs[(i + 1) % planes[n].bound_num][2];
                planes[n].Vertexs[i] = Bounds.inverse() * uB;
    ...
    for (int i = 0; i < m->hfield_ncol[0]; i++)
    {
        ...
        for (auto one_plane : planes)
        {
            ...
            int upper_j = std::min(int((max_y - (pos_y - radius_y)) * m->hfield_nrow[0] / (2 * radius_y)),
                                   m->hfield_nrow[0]);
...
#endif
    memset(current_ctrl_law.Kp, 0, 12 * sizeof(mjtNum));
    memset(current_ctrl_law.Kd, 0, 12 * sizeof(mjtNum));
    memset(current_ctrl_law.qpos_des, 0, 12 * sizeof(mjtNum));
    memset(current_ctrl_law.qvel_des, 0, 12 * sizeof(mjtNum));
    memset(current_ctrl_law.tau_ff, 0, 12 * sizeof(mjtNum));
}
...
// update robot state to shared memory
void publishRobotState()
{
    ...
    simIsDone = false;
    mtx.lock();
    memcpy(measuredState.imuData.quat, d->sensordata, 4 * sizeof(double));
    memcpy(measuredState.imuData.gyro, d->sensordata + 4, 3 * sizeof(double));
    memcpy(measuredState.imuData.acc, d->sensordata + 7, 3 * sizeof(double));
    memcpy(measuredState.jointsState.qpos, d->sensordata + 10, 12 * sizeof(double));
    memcpy(measuredState.jointsState.qvel, d->sensordata + 22, 12 * sizeof(double));
    memcpy(measuredState.jointsState.tau, d->ctrl, 12 * sizeof(double));
    for (int i = 0; i < 4; i++)
    {
        measuredState.force[i] = (int16_t)1.0 * d->sensordata[34 + i];
    }
    mtx.unlock();
    if (lcm_pub.publish("AliengoState", &measuredState) != 0)
    {
...
{
    mtx.lock();
    lcm_planes.plane_num = 13;
    for (int i = 0; i < lcm_planes.plane_num; i++)
    {
        memcpy(lcm_planes.planes[i].A, planes[i].A.data(), 3 * sizeof(double));
        // lcm_planes.planes[i].A[2] += lcm_planes.planes[i].A[0] * d->qpos[0] + lcm_planes.planes[i].A[1] * d->qpos[1] - d->qpos[2];
        // for (int j = 0; j < 4; j++)
        // {
        //     lcm_planes.planes[i].Bs[j][2] -= lcm_planes.planes[i].Bs[j][0] * d->qpos[0] + lcm_planes.planes[i].Bs[j][1] * d->qpos[1];
        //     lcm_planes.planes[i].Vertexs[j][0] -= d->qpos[0];
        //     lcm_planes.planes[i].Vertexs[j][1] -= d->qpos[1];
        // }
        lcm_planes.planes[i].bound_num = planes[i].bound_num;
        for (int n = 0; n < 6; n++)
        {
            memcpy(lcm_planes.planes[i].Bs[n], planes[i].Bs[n].data(), 3 * sizeof(double));
            memcpy(lcm_planes.planes[i].Vertexs[n], planes[i].Vertexs[n].data(), 2 * sizeof(double));
        }
    }
    memcpy(lcm_planes.pos, d->qpos, 3 * sizeof(double));
    memcpy(lcm_planes.quat, d->qpos + 3, 4 * sizeof(double));
    mtx.unlock();
    if (lcm_pub.publish("PerceptionTerrain", &lcm_planes) != 0)
    {
        printf("lcm failed in publishing perception terrain\n");
    }
}

void publishPerceptionLocation()
{
    mtx.lock();
    memcpy(lcm_location.pos, d->qpos, 3 * sizeof(double));
    memcpy(lcm_location.quat, d->qpos + 3, 4 * sizeof(double));
    mtx.unlock();
    if (lcm_pub.publish("PerceptionLocation", &lcm_location) != 0)
    {
        printf("lcm failed in publishing perception terrain\n");
    }
}

// read ctrl from shared_memory, apply it to the robot.
void applyCtrl()
{
}
        ...
        if (m)
        {
            ...
            if (settings.run)
            {
                    ...
                    mju_abs((d->time - simsync) - (tmstart - cpusync)) > syncmisalign)
                {
                    ...
                    for (int i = 0; i < 12; i++)
                    {
                        d->ctrl[i] = current_ctrl_law.Kp[i] * (current_ctrl_law.qpos_des[i] - d->qpos[i + 7]) + current_ctrl_law.Kd[i] * (current_ctrl_law.qvel_des[i] - d->qvel[i + 6]) +
                                     current_ctrl_law.tau_ff[i];
                        d->ctrl[i] = std::min(std::max(d->ctrl[i], -40.0), 40.0);
                    }
                    mj_step2(m, d);
                }
...
public:
    ...
    void handleMessage(const lcm::ReceiveBuffer *rbuf,
                       ...
                       const JointsCmd_lcm *msg)
    {
        mtx.lock();
        ctrl_quene.push(*msg);
        if (m != nullptr && ctrl_quene.size() == 1)
        {
            current_ctrl_law = ctrl_quene.front();
            ctrl_quene.pop();
        }
        mtx.unlock();
    }
...
int main(int argc, const char **argv)
{
    ...
    lcm_rec.subscribe("AliengoJointsCmd", &Handler::handleMessage, &handlerObject);

    PeriodicTaskManager taskManager;
    PeriodicFunction loop_lcm_pub(&taskManager, 0.002, "loop_lcm_pub_robotState", publishRobotState);
    loop_lcm_pub.start();

    PeriodicTaskManager taskManagerPerception;
    PeriodicFunction loop_lcm_pub_perception(&taskManagerPerception, 0.05, "loop_lcm_pub_perception",
                                             publishPerception);
    PeriodicFunction loop_lcm_pub_perceptionLocation(&taskManagerPerception, 0.01, "loop_lcm_pub_perceptionLocation", publishPerceptionLocation);
    loop_lcm_pub_perception.start();
    loop_lcm_pub_perceptionLocation.start();
