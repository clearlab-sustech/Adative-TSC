...
namespace clear
{
      ...
      : nodeHandle_(nodeHandle)
  {
    ...
    mapping_ = std::make_shared<ocs2::CentroidalModelPinocchioMapping>(robot_interface_ptr_->getCentroidalModelInfo());
  ...
  void TrajectorGeneration::fitTraj()
  {
    ...
    ocs2::PinocchioInterface pinocchioInterface = robot_interface_ptr_->getPinocchioInterface();
    ...
    for (size_t i = 1; i < n; i += 3)
    {
      ...
      ocs2::updateCentroidalDynamics(pinocchioInterface, robot_interface_ptr_->getCentroidalModelInfo(), qDesired);
      ...
      if (i == 1)
      {
        auto base_twist = pinocchio::getFrameVelocity(model, data, model.getFrameId(base_name),
      ...
      {
        foot_pos_array[name].push_back(data.oMf[model.getFrameId(name)].translation());
        if (i == 1)
        {
          auto twist = pinocchio::getFrameVelocity(model, data, model.getFrameId(name),
                                                   pinocchio::LOCAL_WORLD_ALIGNED);
    ...
    /* for (size_t i = 0; i < time.size(); i++)
    {
      std::cout << " t: " << time[i] - time.front()
                << " pos: " << pos_t[i].transpose() << " " << cubicspline_pos->evaluate(time[i]).transpose() << "\n";
    } */
    ...
    for (auto name : foot_names)
    {
      ...
      cubicspline_->set_boundary(
          CubicSplineInterpolation::BoundaryType::first_deriv, foot_vel_array[name],
          CubicSplineInterpolation::BoundaryType::second_deriv, vector3_t::Zero());
