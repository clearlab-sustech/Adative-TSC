...
namespace clear
{
  ...
  void TrajectoryStabilization::updateMpcSolution(
      std::shared_ptr<ocs2::PrimalSolution> mpc_sol)
  {
    mpc_sol_buffer.push(mpc_sol);
  }

  void TrajectoryStabilization::updateReferenceBuffer(std::shared_ptr<ReferenceBuffer> referenceBuffer)
  {
  ...
  void TrajectoryStabilization::adapative_gain_loop()
  {
    ...
    while (rclcpp::ok() && run_.get())
    {
      timer_.startTimer();
      if (qpos_ptr_buffer.get() == nullptr || qvel_ptr_buffer.get() == nullptr ||
          mpc_sol_buffer.get() == nullptr || referenceBuffer_.get() == nullptr)
      {
        continue;
      }
      else
      {
        ...
        vf_ptr_->updateMpcSol(mpc_sol_buffer.get());
