...
namespace clear
{
  ...
  void WholeBodyController::update_vector_field(
      const std::shared_ptr<ConstructVectorField::VectorFieldParam> vf)
  {
    base_vector_field_.push(vf);
  }
  ...
  std::shared_ptr<ActuatorCommands> WholeBodyController::optimize()
  {
    ...
    if (base_vector_field_.get().get() == nullptr)
    {
    ...
    if (solver_state == eiquadprog::solvers::EIQUADPROG_FAST_OPTIMAL)
    {
      ...
      matrix6x_t J = matrix6x_t::Zero(6, pinocchioInterface_ptr_->nv());
      pinocchioInterface_ptr_->getJacobia_local(base_name, J);
      /* std::cout << "acc base opt: " << (J * optimal_u.head(pinocchioInterface_ptr_->nv())).transpose() << "\n";
      std::cout << "total_mass: " << pinocchioInterface_ptr_->total_mass() << "\n"; */
    }
    else
    {
      joint_acc_.setZero(actuated_joints_name.size());
  ...
  MatrixDB WholeBodyController::formulateBaseTask()
  {
    ...
    size_t nv = pinocchioInterface_ptr_->nv();
    const auto policy = base_vector_field_.get();
        ...
        rpy_traj.get() != nullptr)
    {
      ...
      std::cout << "feedforward: " << policy->b.transpose() << "\n"; */

      acc_fb = policy->K * x0 + policy->b;

      // std::cout << "acc_fb 0: " << acc_fb.transpose() << "\n";
  ...
  void WholeBodyController::differential_inv_kin()
  {
    ...
    for (size_t k = 0; k < nf; k++)
    {
      ...
      if (!contact_flag[k])
      {
        ...
        vel_m = pinocchioInterface_ptr_->getFrame6dVel_localWorldAligned(foot_name).linear();

        pos_des = foot_traj->evaluate(time_c);
      ...
      else
      {
        if (joint_acc_.size() == nj)
        {
          ...
          for (size_t i = 0; i < 3; i++)
          {
            ...
            actuator_commands_->vel(idx[i]) = jnt_vel(idx[i]) + dt_ * joint_acc_(idx[i]);
          }
        }
      }
    }
