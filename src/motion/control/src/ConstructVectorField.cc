...
namespace clear
{
        ...
        base_name_(base_name)
  {
    ...
    weight_.diagonal() << 40, 40, 50, 0.1, 0.1, 0.2, 30, 30, 50, 0.1, 0.1, 0.1;
  ...
  void ConstructVectorField::update_mode_schedule(
  ...
  void ConstructVectorField::add_linear_system(size_t k)
  {
    ...
    if (has_sol_ &&
        k < solution_.size() - 1)
    { // the last one solution has no data of u
  ...
  std::shared_ptr<ConstructVectorField::VectorFieldParam> ConstructVectorField::compute()
  {
    vector_field_ptr = nullptr;
    ...
    {
      has_sol_ = true;
      vector_field_ptr = std::make_shared<VectorFieldParam>();
      matrix_t P(6, 12);
      P.setZero();
      ...
      vector_field_ptr->K = P * (A + B * solution_[0].K);
      vector_field_ptr->b = P * (B * solution_[0].k + drift);
      vector3_t omega_des = getJacobiFromRPYToOmega(rpy) * rpy_dot_des;
      vector_field_ptr->b.tail(3) -= Ig_.inverse() *
