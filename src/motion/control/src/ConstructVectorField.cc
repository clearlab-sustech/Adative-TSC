...
namespace clear
{
  ...
  void ConstructVectorField::updateMpcSol(std::shared_ptr<ocs2::PrimalSolution> mpc_sol)
  {
  ...
  void ConstructVectorField::add_linear_system(size_t k)
  {
    ...
    auto contact_flag =
        quadruped::modeNumber2StanceLeg(mpc_sol_buffer.get()->modeSchedule_.modeAtTime(time_k));
  ...
  ConstructVectorField::compute()
  {
    ...
    size_t N = min(rpy_traj->duration(), 0.6) / dt_;
    ocp_.resize(N + 1);
