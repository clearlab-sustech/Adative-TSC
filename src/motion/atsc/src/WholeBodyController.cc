...
void WholeBodyController::update_base_policy(
    const std::shared_ptr<ConstructVectorField::VectorFieldCoeffs> policy) {
  base_policy_.push(policy);
}

void WholeBodyController::formulate() {
  mode_.push(refTrajBuffer_->get_mode_schedule()->getModeFromPhase(0.0));
...
MatrixDB WholeBodyController::formulateBaseTask() {
  ...
  auto rpy_traj = refTrajBuffer_->get_base_rpy_traj();
...
void WholeBodyController::differential_inv_kin() {
  auto foot_traj_array = refTrajBuffer_->get_foot_pos_traj();
  auto pos_traj = refTrajBuffer_->get_base_pos_traj();
  ...
  auto contact_flag = legged_robot::modeNumber2StanceLeg(mode_.get());
