...
public:
  struct VectorFieldCoeffs {
    matrix_t K;
    vector_t b;
  };

public:
  ConstructVectorField(Node::SharedPtr nodeHandle,
  ...
  std::shared_ptr<VectorFieldCoeffs> compute();
...
private:
  ...
  std::shared_ptr<VectorFieldCoeffs> feedback_law_ptr;
