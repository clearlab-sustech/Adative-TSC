...
MotionManager::MotionManager() : Node("MotionManager") {
  this->declare_parameter("/config_file", "/home/clear-lab/ysp/exp_ocs2/src/asserts/a1/config.yaml");
}
...
void MotionManager::init() {
  ...
  // gaitSchedulePtr_ =
  // std::make_shared<GaitSchedule>(this->shared_from_this());

  trajGenPtr_ = std::make_shared<TrajectorGeneration>(this->shared_from_this());

  trajectoryStabilizationPtr_ = std::make_shared<TrajectoryStabilization>(
      this->shared_from_this(), trajGenPtr_->getRobotInterface());
...
void MotionManager::innerLoop() {
  ...
  while (rclcpp::ok() && run_.get()) {
    ...
    if (gaitSchedulePtr_->getCurrentGaitName() == "trot") {
      trajGenPtr_->setVelCmd(vector3_t(0.2, 0.1, 0.0), 0.2);
    } */
    ...
    trajectoryStabilizationPtr_->updateMpcSolution(trajGenPtr_->getMpcSol());
...
namespace clear
{
  ...
  void MotionManager::init()
  {
    ...
    trajectoryStabilizationPtr_ =
        std::make_shared<TrajectoryStabilization>(this->shared_from_this(), trajGenPtr_->getRobotInterface());
  ...
  void MotionManager::innerLoop()
  {
    ...
    while (rclcpp::ok() && run_.get())
    {
      ...
      trajectoryStabilizationPtr_->updateReferenceBuffer(trajGenPtr_->getReferenceBuffer());

      trajectoryStabilizationPtr_->updateMpcSolution(
          trajGenPtr_->getMpcSol());
