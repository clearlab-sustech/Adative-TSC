...
StateEstimationLKF::StateEstimationLKF(std::string config_yaml)
    : Node("estimator") {
  ...
  RCLCPP_INFO(this->get_logger(), "use odom: %s", use_odom_ ? "true" : "false");
...
void StateEstimationLKF::publish_estimated_states() {
  trans::msg::EstimatedStates msg;
  msg.joint_states = *joint_state_msg_buffer.get();
  if (use_odom_) {
    msg.odom = *odom_msg_buffer.get();
  } else {
    auto qpos_ptr = qpos_ptr_buffer.get();
    auto qvel_ptr = qvel_ptr_buffer.get();
    msg.odom.header.frame_id = msg.joint_states.header.frame_id;
    msg.odom.header.stamp = this->now();

    msg.odom.pose.pose.position.x = (*qpos_ptr)[0];
    msg.odom.pose.pose.position.y = (*qpos_ptr)[1];
    msg.odom.pose.pose.position.z = (*qpos_ptr)[2];
    msg.odom.pose.pose.orientation.w = (*qpos_ptr)[6];
    msg.odom.pose.pose.orientation.x = (*qpos_ptr)[3];
    msg.odom.pose.pose.orientation.y = (*qpos_ptr)[4];
    msg.odom.pose.pose.orientation.z = (*qpos_ptr)[5];

    msg.odom.twist.twist.linear.x = (*qvel_ptr)[0];
    msg.odom.twist.twist.linear.y = (*qvel_ptr)[1];
    msg.odom.twist.twist.linear.z = (*qvel_ptr)[2];
    msg.odom.twist.twist.angular.x = (*qvel_ptr)[3];
    msg.odom.twist.twist.angular.y = (*qvel_ptr)[4];
    msg.odom.twist.twist.angular.z = (*qvel_ptr)[5];
  }
  res_publisher_->publish(msg);
}
