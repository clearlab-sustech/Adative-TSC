...
// #define FOOT_DOWN_WITH_ZERO_ACC  // if using the x-t function that make touch down acc of foot to zero
...
namespace Interpolate
{
...
#ifndef FOOT_DOWN_WITH_ZERO_ACC
  ...
  y_t cubicBezier(y_t y0, y_t yf, x_t x)
  {
    ...
    assert(x >= 0 && x <= 1);
    y_t yDiff = yf - y0;
    x_t bezier = x_t(6) * x * x * x * x * x - x_t(15) * x * x * x * x + x_t(10) * x * x * x;
    return y0 + bezier * yDiff;
  }
  ...
  y_t cubicBezierFirstDerivative(y_t y0, y_t yf, x_t x)
  {
    ...
    assert(x >= 0 && x <= 1);
    y_t yDiff = yf - y0;
    x_t bezier = x_t(30) * x * x - x_t(60) * x * x * x + x_t(30) * x * x * x * x;
    return bezier * yDiff;
  }
  ...
  y_t cubicBezierSecondDerivative(y_t y0, y_t yf, x_t x)
  {
    ...
    assert(x >= 0 && x <= 1);
    y_t yDiff = yf - y0;
    x_t bezier = x_t(60) * x - x_t(180) * x * x + x_t(120) * x * x * x;
    return bezier * yDiff;
  }
