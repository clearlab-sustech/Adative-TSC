...
#ifndef CLIMBSTAIRS_POPLARMPC_H
#define CLIMBSTAIRS_POPLARMPC_H
...
namespace poplar_mpc
{
    ...
    using Vec4 = typename Eigen::Matrix<RealNum, 4, 1>;
    ...
    {
    public:
        explicit PoplarMPC(size_t horizon, RealNum dt);
        ...
        void setCurrentFootholds(vector<Vec3> footholds);

        void setSwingTimeRemain(Vec4 swtr);

        void setStanceTimeRemain(Vec4 sttr);

        void setNorminalFootholds(Vec12 fh_n);
        
        void setFrictionCoefficient(RealNum mu);

        void setMaxForce(RealNum fmax);

        void setDesiredFootZPos(RealNum zd);
        ...
        ConstVecRef getCurrentDesiredFootholds();
    ...
    private:
        ...
        size_t _horizon;
        RealNum _dt, _gravity, _mu, _fmax, _zd;
        RealNum _mass;
        RealNum _swingVel_max = 2.0;
        Mat3 _inertia;
