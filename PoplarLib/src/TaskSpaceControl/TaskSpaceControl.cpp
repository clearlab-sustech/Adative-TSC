...
TaskSpaceControl::TaskSpaceControl(RobotWrapper &robot) : _robot(robot), _u_dims(0) {
}

void TaskSpaceControl::addTask(Task* task_ptr) {
    if (!existTask(task_ptr->name())) {
        _tasks.push_back(task_ptr);
    } else {
        throw runtime_error("Task " + task_ptr->name() + " exists");
    }
}

void TaskSpaceControl::addLinearConstraint(LinearConstraints* constraints_ptr) {
    if (!existLinearConstraint(constraints_ptr->name())) {
        _linearConstraints.push_back(constraints_ptr);
    } else {
        throw runtime_error("Constraints " + constraints_ptr->name() + " exists");
    }
}

bool TaskSpaceControl::existLinearConstraint(string name) {
...
ConstVecRef TaskSpaceControl::getOptimalQacc() {
    return ConstVecRef(optimal_u.head(_robot.nv()));
}
...
bool TaskSpaceControl::existTask(string name) {
    for (auto it = _tasks.begin(); it != _tasks.end(); it++) {
        if ((*it)->name() == name) {
            return true;
        }
